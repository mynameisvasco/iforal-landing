<section class="position-relative py-24 bg-white">
	<img
		class="position-absolute top-0 start-0 bottom-0 h-100 col-8"
		src="pstls-assets/images/contacts/shadow-small.png"
		alt=""
	/>
	<div class="container">
		<div class="row align-items-center">
			<div class="col-12 col-md-6 mb-12 mb-md-0">
				<div class="mw-md">
					<h3 class="display-4 lh-sm font-heading">Stay in touch with us</h3>
				</div>
			</div>
			<div class="col-12 col-md-6" style="z-index: 200">
				<div class="row">
					<div class="col-12 mb-12">
						<h4 class="display-6 font-heading">Email</h4>
						<a class="text-decoration-none text-dark" href="mailto:contacto@iforal.pt">
							iforal@letras.ulisboa.pt
						</a>
					</div>
					<div class="col-12">
						<h4 class="display-6 font-heading">Social Networks</h4>
						<div class="d-flex">
							<a class="text-decoration-none" href="#">
								<img src="pstls-assets/images/contacts/facebook.png" alt="" />
							</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>
